 ...
 function validateField(fieldId, submission, cb) {
   init(function(err){
      ...
      initSubmission(submission, function (err) {
        ...
        var fieldDef = fieldMap[fieldId];
        getFieldValidationStatus(submissionField, fieldDef, function (err, res) {
          if (err) return cb(err);
          var ret = {validation: {}};
  ...
  function validatorDateTime  (fieldValue, fieldDefinition, cb) {
    ...
    switch (fieldDefinition.fieldOptions.dateTimeUnit)
      {
      ...
      case FIELD_TYPE_DATETIME_DATETIMEUNIT_DATETIME:
        ...
        break;
      default:
        return cb(new Error("Invalid dateTime fieldtype " + fieldOptions.dateTimeUnit));
        break;
      }
  }
...
function cvtTimeToSeconds(timeStr) {
  var seconds = 0;
  if (typeof fieldValue === "string") {
    var parts = fieldValue.split(':');
    valid = (parts.length === 2) || (parts.length === 3);
