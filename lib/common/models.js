  ...
  return {
    "init": function (conn, config) {
      ...
      var dataSourceCacheSchema = new Schema({
        ...
        //TODO: Include Error Handling. (C) Or (W) at best From Tech Spec
        currentStatus: {
        ...
        data: {
          fieldType: {type: String, required: true, enum: ["singleChoice", "multiChoice"]},
          options: [
            dataSourceCacheOption
          ]
        }
      }, schemaOptionsNoId);

      dataSourceCacheSchema.path('data.options').validate(function(options){
        return options.length > 0;
