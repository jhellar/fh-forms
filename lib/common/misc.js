 ...
 *
 * 0 and false are considered valid field value entries.
 *
 * @param  {array} fieldValues Array of field values to filter.
...
function mapSubmissionValsToFormsFields(submission, formJson, cb) {
   ...
   * Or the field definition is an admin field that is in the formSubmitted against.
  ...
  formJson.pages = _.map(formJson.pages, function(page) {
    page.fields = _.map(page.fields, function(field) {
      ...
      if (subFieldMatch) {
        subFieldMatch.fieldValues = filterOutNullData(subFieldMatch.fieldValues);
...
/**
 * sanitiseJSON - Helper function to remove any invalid data types from incoming json.
 *
 * @param  {type} jsonToSanitise description
 * @return {type} sanitisedJSON  description
 */
...
module.exports.sanitiseJSON = sanitiseJSON;
...
module.exports.filterOutNullData = filterOutNullData;
