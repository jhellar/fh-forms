...
module.exports = function (fieldDefinition, fieldSubmission, existingFieldSumbission){
  if (process.env.FH_FORMS_DEBUG) console.log('fieldDefinition:', fieldDefinition, 'fieldSubmission:', fieldSubmission, ' existingFieldSumbission:', existingFieldSumbission);
  ...
  return{
    ...
    "checkFilePlaceholder" : function(fileEntry, cb){
      ...
      if(fileEntry.indexOf("filePlaceHolder") > -1){//TODO abstract out to config
        return cb();
      } else if (existingFieldSumbission && existingFieldSumbission.indexOf(fileEntry) > -1){
        // existing file object id, ok
        if (process.env.FH_FORMS_DEBUG) console.log('existing field ', fileEntry, ', passes validation');
        return cb();
      } else {
