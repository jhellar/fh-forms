...
function populateSubmissionFileData(params){
  ...
  //Mapping files to submission field values
  submissionForm.pages = _.map(submissionForm.pages, function(page){
...
/**
 * Cacheing any files needed for a submission to the file system.
 *
 * @param params
 *  - submission
 *  - connections
...
function mergeSubmissionFiles(params, cb) {
  ...
  var fileFieldDetails = _.map(form.pages, function(page){
    return _.filter(page.fields, function(field){
  ...
  async.mapSeries(filesToDownload, function(fileIdOrUrl, cb){
    ...
    submission.formSubmittedAgainst =  populateSubmissionFileData({
      ...
      downloadUrl: params.options.downloadUrlHost,
      submission: submission
    });
...
function removeCachedFiles(params, cb){
  ...
  logger.debug("Removing Cached Files For PDF Generation: ", params.submissionFiles);
