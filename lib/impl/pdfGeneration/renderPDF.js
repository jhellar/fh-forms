...
function createPhantomSession(cb) {
  ...
  try {
    logger.info('Creating new phantom session');
    phantom.create({
      format: 'a4',
      onExit: function(code, signal) {
        logger.info('phantom session exited with code', code);
...
function loadPdfTemplate(params, cb) {
  ...
  fs.readFile(templatePath, function(err, templateFile) {
    ...
    //TODO: This should really change. The Template Should Be Loaded From Ngui. Forms are not used in studio.
    var studioTemplatePath = url.parse("http://" + params.location + params.pdfTemplateLoc);
    request(url.format(studioTemplatePath), function (error, response, body) {
      ...
      if (response.statusCode === 200) {
        ...
        return cb("Non 200 response from get template " + JSON.stringify(body));
      }
    });
  });
}
