...
function addDataToUsersGroups(connections, data, restrictToUser, valueToAdd, cb) {
  ...
  groupsModel.update(query, {$push: push}, {multi: true}, function(err, numAffected, rawResponse) {
    if (err) return cb(err);
...
console.log('>>>>>>validateDataAllowedForUser() - query: ', query);
...
console.log("Access to " + data + ": " + valueToCheck + ", allowed to user: " + restrictToUser + ", by groups: ", dataList);
      return cb(); // return valid - found user and valueToCheck in a group
    } else {
      return cb(new Error("No access to " + data + ": " + valueToCheck + ", allowed to user: " + restrictToUser + ", due to group permissions"));
    }
  });
}
