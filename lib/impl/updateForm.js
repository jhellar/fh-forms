...
module.exports = function updateForm(connections, options, formData, cb) {
   ...
   *
   * All field codes must have a length > 0, otherwise don't save it.
   * @param cb
   */
  function validatefieldCodes(cb){
    ...
    if(duplicateFieldCode){
      return cb(new Error("Duplicate Field Codes Detected. Field Codes Must Be Unique In A Form."))
    }

    if(invalidFieldCode){
      return cb(new Error("Invalid Field Code. Field Codes Must Be A String."));
    }
