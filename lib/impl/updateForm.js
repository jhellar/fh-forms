...
module.exports = function updateForm(connections, options, formData, cb) {
  ...
  function updateRules(form, cb) {
    ...
    function scanForInvalidFields(type, rule, callback){
      ...
      async.each(targetField, function(targetField, cb){
        ...
        isFieldStillValidTarget(targetField, formPages, function(invalidField){
          if(invalidField){
            console.log("INVALID FIELD", targetField);
    ...
    function updateOrDeleteRules(cb){
      console.log("BEFORE deleteRules, ", fieldRulesToDelete, pageRulesToDelete);
    ...
    function updateFormRules(cb){
      ...
      function filterConditionalStatements(rule){
        return _.filter(rule.ruleConditionalStatements, function(ruleCondStatement){
          ...
          if(invalidFields[sourceField]){
             ...
             */
            console.log("INVALID FIELD ", sourceField, formData.ruleDeletionFlags, formData.ruleDeletionFlags[sourceField]);
      ...
      console.log("BEFORE FINISHED UPDATING FIELD RULES, ", form.fieldRules, form.pageRules, invalidFields);
      ...
      fieldRulesToDelete = _.filter(updatedFieldRules, function(fieldRule){
        ...
        console.log("CHECKING FIELD, ", targetFields.length === 0, conditionalStatements.length === 0, rulesFlaggedForDeletion, fieldRuleId, rulesFlaggedForDeletion[fieldRuleId]);
      ...
      pageRulesToDelete = _.filter(updatedPageRules, function(pageRule){
        ...
        console.log("CHECKING PAGE, ", conditionalStatements.length === 0, rulesFlaggedForDeletion[pageRuleId]);
      ...
      console.log("BEFORE FINISHED UPDATING FIELD RULES, ", updatedFieldRules, updatedPageRules, fieldRulesToDelete, pageRulesToDelete);

      form.fieldRules = updatedFieldRules;
      form.pageRules = updatedPageRules;

      console.log("FINISHED UPDATING FIELD RULES, ", form.fieldRules, form.pageRules);
