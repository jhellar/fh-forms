...
module.exports = function updateForm(connections, options, formData, cb) {
  ...
  function validateDuplicateName(cb){
    ...
    formModel.count(query, function(err, count){
      ...
      //If the number of found forms is > 1, then there is another form with the same name. Do not save the form.
      if(count > 0){
        return cb(new Error("Duplicate form with name " + formName + " found."));
