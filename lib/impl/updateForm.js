...
module.exports = function updateForm(connections, options, formData, cb) {
  ...
  function updateRules(form, cb) {
    console.log("IN updateRules: ", form);
    ...
    function scanForExistingFields(type, rule, callback){
      console.log("IN scanForExistingFields", type, rule);
      ...
      if(fieldAndPageIds.indexOf(targetField) === -1){
        console.log("Target Field or page gone: ", fieldAndPageIds, targetField);
        ...
        console.log("Target Field or page Exists: ", fieldAndPageIds, targetField);
        async.each(rule.ruleConditionalStatements, function(ruleConStatement, ruleCb){

         var sourceId = ruleConStatement.sourceField;
         sourceId = sourceId.toString();
          console.log("CHECKING RULE CON: ", ruleConStatement, fieldAndPageIds, sourceId);
         if(fieldAndPageIds.indexOf(sourceId) === -1){
          ...
          if(targetExists === false){
            console.log("Conditional targetExists: ", targetExists);
            if(type === models.MODELNAMES.FIELD_RULE){
              fieldRulesToDelete.push(rule._id);
            } else {
              pageRulesToDelete.push(rule._id);
    ...
    function deleteRules(type, cb){
      console.log("IN deleteRules " , type);
      if(type === models.MODELNAMES.FIELD_RULE){
    ...
    async.each(formPages, function (formPage, callbackPage) {
      ...
      async.parallel([
        ...
        console.log("fieldRulesToDelete: " , fieldRulesToDelete);
        console.log("pageRulesToDelete: " , pageRulesToDelete);
  ...
  function doUpdate(formData, cb) {
    formModel.findById(formData._id).populate("fieldRules pageRules pages").exec(function (err, doc) {
      if (err) return cb(err);

      //Now populate the fields in each page
      formModel.populate(doc, {"path": "pages.fields", "model": fieldModel, "select": "-__v -fieldOptions._id"}, function (err, doc) {
        if (err) return cb(err);
