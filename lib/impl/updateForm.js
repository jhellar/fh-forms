  ...
  */
  function refactorRules(form, cb) {
    ...
    function checkRulesWithRepeatingSections() {
      var fieldSectionMapping = getSectionMap(form);
      fieldSectionMapping = _.mapObject(fieldSectionMapping, getFieldById);

      form.fieldRules.forEach(function(fieldRule) {
        var sectionIds = _.chain(fieldRule.ruleConditionalStatements)
      ...
      function getFieldById(id) {
        ...
        _.find(form.pages, function(page) {
          return _.find(page.fields, function(f) {
            var found = f._id.toString() === id;
            if (found) {
              field = f;
            }
