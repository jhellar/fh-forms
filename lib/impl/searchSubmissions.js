...
function queryBuilderIsNot(clause) {
  return { "$not": clause.value};
}

function constructValueQuery(clause) {
  var queryBuilder = queryMap[clause.restriction] || function () {console.log("UNNKNON RESTRICTION"); return {};};
  return queryBuilder(clause);
}
...
exports.queryBuilder = function buildQuery(params, cb) {
  ...
  if(params.queryMeta && params.queryMeta.clauses) {
    async.eachSeries(params.queryMeta.clauses, function (metaQuery) {
      var add = {};
      add[metaQuery.metaName] = constructValueQuery(metaQuery);
      query["$and"].push(add);
    });
  }
  ...
  async.eachSeries(params.queryFields.clauses, function (clause, cb) {
    ...
    var fieldValues = constructValueQuery(clause);
