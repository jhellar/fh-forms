  ...
  validateParams(function(err) {
    ...
    exports.queryBuilder(params, function(err, query) {
      ...
      function reformatFormIdAndName(submission, cb) {
        submission.formName = submission.formSubmittedAgainst.name;
        return cb(undefined, submission);
      }

      formSubmissionModel
        .find(query)
        .select({"formSubmittedAgainst.name": 1, "_id": 1, "formId": 1, "appId": 1, "appEnvironment": 1, "formFields": 1})
        .sort({submissionCompletedTimestamp: -1})
