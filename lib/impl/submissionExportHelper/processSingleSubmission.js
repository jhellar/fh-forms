...
function generateRepeatingSectionCSV(section, mergedFields, submittedFields) {
  ...
  _.each(fieldsInSection, function(fieldInSection) {
    ...
    var thisFieldInSection = _.filter(submittedFields, function(submittedField) {
      return submittedField.fieldId.toJSON() === fieldInSection._id;
    });

    if (!addedSectionBreaks) {
      _.each(thisFieldInSection, function(field) {
        field.sectionIndex = field.sectionIndex || 0;
        data[field.sectionIndex] = [];
      });
      addedSectionBreaks = true;
    }

    _.each(thisFieldInSection, function(thisField) {
      data[thisField.sectionIndex].push({fieldModel:fieldInSection, fieldValuesInSections: thisField});
    });

    for (var i = 0; i < section.fieldOptions.definition.maxRepeat; i++) {
      var dataForIndex = data[i];
      if (!dataForIndex) {
        var emptyFieldValue = {sectionIndex: i, fieldValues: []};
        data[i] = [{fieldModel:fieldInSection, fieldValuesInSections: emptyFieldValue}];
      }
    }

  });

  return {data: _.flatten(data), idsOfFieldsInSection: idsOfFieldsInTheSection};
}
