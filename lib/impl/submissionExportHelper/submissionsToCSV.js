...
// Adding an endsWith check which we need later on, save us using a new module
...
var metaDataHeaders = ['formName', 'formId', '_id', 'submissionCompletedTimestamp', 'appCloudName', 'deviceId', 'deviceIPAddress', 'updatedTimestamp'];
...
function submissionsToCSV(params, cb) {
    ...
    /*
     * Here we need to parse for escaped characters, Timestamps are the exception to this because of their format
     */
    function addMetadataValues(line, sub) {
        _.each(metaDataHeaders, function(headerName) {
            if (headerName === 'submissionCompletedTimestamp' || headerName === 'updatedTimestamp') {
                line += sub[headerName] + ',';
            } else {
    ...
    function processSubmission(sub, cb) {
        ...
        if (!csvs[csvKey]) {
            ...
            _.map(metaDataHeaders, function(headerName) {
                csv += headerName + ',';
            });
        ...
        line += addMetadataValues(line, sub);
