...
var submitFormFile = function(connections, options, cb) {
  ...
  function saveFileStream(cb){
    ...
    fileHandler.saveFile(connections.databaseConnection, submission.fileName, submittedFileStream, {"decodeBase64" : submission.decodeBase64}, function(err, saveResult){
      if(err) return cb(err);
