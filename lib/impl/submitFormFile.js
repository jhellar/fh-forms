...
var submitFormFile = function(connections, options, cb) {
  ...
  function saveFileStream(cb){
    ...
    var fileNameToSave = submissionToUse.formFields[submissionFieldIndex].fieldValues[filePlaceholderIndex].fileName;

    var fileExtension = mime.extension(submissionToUse.formFields[submissionFieldIndex].fieldValues[filePlaceholderIndex].fileType);

    if(fileNameToSave.indexOf("." + fileExtension) === -1){
      fileNameToSave + "." + fileExtension;
    }

    fileHandler.saveFile(connections.databaseConnection, fileNameToSave , submittedFileStream, {"decodeBase64" : submission.decodeBase64}, function(err, saveResult){
      if(err) return cb(err);
