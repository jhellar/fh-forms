...
var submitFormFile = function(connections, options, cb) {
  ...
  function saveFileStream(cb){
    ...
    fileDetails = submissionToUse.formFields[submissionFieldIndex].fieldValues[filePlaceholderIndex];

    var fileNameToSave = fileDetails.fileName;
    var fileExtension = mime.extension(fileDetails.fileType);
    var fileGroupId = fileDetails.groupId;
    ...
    if(fileGroupId){
      saveFileOptions.groupId = fileGroupId;
    }
  ...
  function updateSubmission(cb){
    ...
    if(!fileDetails.groupId){ // The file groupId already exists, do not allow it to be set. No save of the model is required.
