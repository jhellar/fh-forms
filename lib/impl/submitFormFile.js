...
var submitFormFile = function(connections, options, cb) {
  ...
  function findPlaceholder(cb){
    ...
    var indexOfFilePlaceholder = submissionEntry.fieldValues.findIndex(function(fileObject){
      return submission.fileId.toString() === fileObject.toString();
    });
  ...
  function saveFileStream(cb){
    ...
    fileHandler.saveFile(connections.databaseConnection, submissionToUse.formFields[submissionFieldIndex].fieldValues[filePlaceholderIndex].fileName, submittedFileStream, {"decodeBase64" : submission.decodeBase64}, function(err, saveResult){
      if(err) return cb(err);
