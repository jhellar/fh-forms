...
var submitFormData = function(connections, options, cb) {
  ...
  function validateDataAgainstCurrentForm(currentFormJSON, cb){
    ...
    // TODO:
    //  - if model exists, check file fields from client against db model
    //  - throw error if unknown file id added
    //  - remove any missing file ids (and remove file from gridfs later^^^)
    //  - continue with noraml validation below
