...
var submitFormData = function(connections, options, cb) {
  ...
  function saveSubmission(cb){
    ...
    if (formSubmission) {
      // update
      _.each(formSubmission.formFields, function(formField){
        var updatedFieldData = _.find(submissionData.formFields, function(updatedField){
          return updatedField.fieldId.toString() === formField.fieldId.toString();
        }) || [];

        formField.fieldValues = updatedFieldData.fieldValues || formField.fieldValues;
        formField.markModified('fieldValues');
      });
    } else {
      // create
  ...
  function validateSubmissionParams(cb){
    ...
    validateOptions.has("submission", function(failure){
      ...
      validateFormFields.has("formId", "appId", "appCloudName", "appEnvironment", "deviceId", "deviceIPAddress", "comments", "formFields", "deviceFormTimestamp", function(failure){
        if(failure) return cb(new Error("Invalid paramaters for form submission, aborting. " + JSON.stringify(failure) ));
  ...
  function validateSubmissionData(cb){
    ...
    getForm(connections, {_id: submissionData.formId, expectDataSourceCache: true}, function(err, mostRecentSubmissionForm){
