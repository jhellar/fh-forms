...
function buildSubmissionReceivedMessage(subscribers, formName, formSubmission) {
  ...
  formSubmission.formFields.forEach(function(formField) {
    ...
    pages[pageId][sectionId][formField.sectionIndex] = pages[pageId][sectionId][formField.sectionIndex] || [];
    pages[pageId][sectionId][formField.sectionIndex].push(formField);
  });
      ...
      if (repSections) {
        if (repSections.length === 1) {
          ...
          repSections.forEach(function(repSection, index) {
            msg.submittedFields.push(getField(section).name + ' - ' + (index + 1) + ':');
