...
function list(connections, params, cb){
  ...
  if(params.listDataSourcesNeedingUpdate && !params.currentTime && currentTime.toString() !== "Invalid Date"){
    return cb(buildErrorResponse({error: new Error("An currentTime Date Object Is Required To List Data Sources Requiring Update"), code: ERROR_CODES.FH_FORMS_INVALID_PARAMETERS}));
  }
      ...
      lookUpDataSources(connections, {
        ...
        if(params.listDataSourcesNeedingUpdate){
          dataSources = _.filter(dataSources, function(ds){
            return !ds.cache[0] || !ds.cache[0].lastRefreshed || new Date((new Date(ds.cache[0].lastRefreshed).valueOf() + ds.refreshInterval)) <= currentTime;
          });
        }
