 ...
 * @param cb
 */
function updateDataSourceEntry(params, dataSourceData, cb){
  ...
  } else if(dataSourceData.dataError || dataSourceData.error){
  ...
  //The New Cache Should Now Be Valid
  dataSourceDocument.validate(function(err){
    ...
    if(!dataSourceData.error && !dataSourceData.dataError){
      //Mark The Submission As Refreshed If There Is No Error
...
module.exports = function updateCache(connections, dataSources, params, cb){
  ...
  async.waterfall([
    ...
    async.apply(updateDataSourceCaches, params),
