...
var updateSubmissionFile = function(connections, options, cb) {
  ...
  var submissionToUpdate = undefined;
  var foundFileValue = undefined;
  ...
  console.log("update submission File FH_FORMS ", submission, fileDetails);
  ...
  function saveFileStream(cb){
    ...
    console.log("PARAMS FOR FILE SAVE ",fileNameToSave , submittedFileStream, saveFileOptions);
    ...
    fileHandler.saveFile(connections.databaseConnection, fileNameToSave , submittedFileStream, saveFileOptions, function(err, saveResult){
      console.log("SAVE FILE RESULT ***************** ", err, saveResult, "*******************");
      if(err) return cb(err);
