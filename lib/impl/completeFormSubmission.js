    ...
    async.series([findSubmission, checkSubmissionComplete], function(err){
      ...
      if("complete" === completeStatus.status) {
        updateSubmission(function (err) {
          ...
          notification.sendNotification(connections, completeStatus, function (err) {
            return cb(undefined, completeStatus);
          });
        });
      } else {
        logger.warning('completeFormSubmission() - completeSubmission attempted but submission still pending: ', completeStatus);
        return cb(undefined, completeStatus);
      }
    });
  });
