...
module.exports = function (connections, options, themeData, cb) {
  ...
  function validateDuplicateName(cb){
    ...
    themeModel.count(query, function(err, count){
      ...
      //If the number of found theme is > 1, then there is another theme with the same name. Do not save the theme.
      if(count > 0){
        return cb(new Error("Duplicate theme with name " + themeName + " found."));
