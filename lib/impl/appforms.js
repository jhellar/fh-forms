...
/*
 * getAppFormsForApp(connections, options, appId, cb)
 *
 *    connections: {
 *       mongooseConnection: ...
 *    }
...
function getAppFormsForApp(connections, options, appId, cb) {
  var conn = connections.mongooseConnection;
  var appFormsModel = models.get(conn, models.MODELNAMES.APP_FORMS);
  appFormsModel.findOne({appId:appId}).populate('forms').exec(function (err, af) {
    if (err) return cb(err);
    return cb(null, af || []);
  });
};
