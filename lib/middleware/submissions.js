...
function updateSubmissionFile(req, res, next){
  ...
  logger.debug("Middleware updateSubmissionFile ", {params: fileDetails});
...
function submitFormFile(req, res, next){
  var params = {
    submission: {
      ...
      fileStream: req.appformsResultPayload.data.submission.fileStream,
...
function exportSubmissions(req, res, next){
  ...
  forms.exportSubmissions(req.connectionOptions, params, function(err, submissionCsvValues){
    ...
    logger.debug("Middleware exportSubmissions submissionCsvValues", {submissionCsvValues: submissionCsvValues});
...
function _processExportResponse(csvs, res, next){
  logger.debug("_processExportResponse ", {csvs: csvs});
