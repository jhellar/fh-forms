...
var dataSourceImpl = require('./impl/dataSources.js');
var dataTargetImpl = require('./impl/dataTargets.js');
...
// forms.getSubmissionFile({"uri": mongoUrl}, {"_id" : req.params.fileGroupId}, function(err, result){
  ...
  dataSources: {
    ...
    updateCache: function(options, params, cb){
      forms.initConnection(options, function(err, connections) {
        if(err) return cb(err);

        return dataSourceImpl.updateCache(connections, params, cb);
      });
    },
...
logWrapper.addLoggingToFunctions(forms, "forms.", ["deleteAppRefrences", "setLogger", "initConnection", "getSubmissions", "submissionSearch"]);
