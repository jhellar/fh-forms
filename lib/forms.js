...
// forms.getSubmissionFile({"uri": mongoUrl}, {"_id" : req.params.fileGroupId}, function(err, result){
  ...
  createAppConfig: function (options, params, cb) {
    ...
    self.initConnection(options, function(err, connections) {
      if(err) return cb(err);
      return appConfig.createAppConfig(connections, options, params, cb);
    });
  },
  ...
  updateAppConfig: function (options, params, cb) {
    ...
    self.initConnection(options, function(err, connections) {
      if(err) return cb(err);
      return appConfig.updateAppConfig(connections, options, params, cb);
    });
  },
  ...
  getAppConfig: function (options, params, cb) {
    ...
    self.initConnection(options, function(err, connections) {
      if(err) return cb(err);
      return appConfig.getAppConfig(connections, options, params, cb);
    });
  },
  ...
  deleteAppConfig: function (options, params, cb) {
    ...
    self.initConnection(options, function(err, connections) {
      if(err) return cb(err);
      return appConfig.deleteAppConfig(connections, options, params, cb);
    });
  },

  getAppClientConfig: function (options, params, cb) {
    var self = this;
    logger.log(logger.DEBUG, "requesting client config for appId: ", params.appId);
    self.initConnection(options, function(err, connections) {
      if(err) return cb(err);
      logger.log(logger.DEBUG, "initialised connect for request of client config for appId: ", params.appId);
      return appConfig.getAppConfig(connections, options, params, function (err, result) {
        if (err) {
          logger.log(logger.ERROR, 'error requesting app config: ', err);
          return cb(err);
        }
        logger.log(logger.DEBUG, 'returning app client config: ', result);
